...
import {
  ...
  parseNtfyMessages,
  ntfyPoll,
} from '../../scripts/lib/live-driver.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
export const repoRoot = path.resolve(__dirname, '..', '..');

// randomTopic / writeUserConfig / parseNtfyMessages / ntfyPoll now live in the
// production driver library (scripts/lib/live-driver.mjs) so the live-* drivers
// no longer import from tests/. Re-export them here so e2e tests keep a single
// import site (./helpers.mjs) and the dependency points tests → scripts only.
export { randomTopic, writeUserConfig, parseNtfyMessages, ntfyPoll };
...
// Remove dedup locks so the same source can fire twice within the dedup
  ...
  try {
    for (const name of fs.readdirSync(dir)) {
      if (name.startsWith(`.lock-${source}`)) {
        try { fs.unlinkSync(path.join(dir, name)); } catch { /* gone already */ }
      }
    }
  } catch { /* no config dir yet — nothing to clear */ }
}
